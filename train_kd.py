...
def val(epoch):
	...
	exModelName = r"D:/trainTemp/handBag/cnn_kd50_112/" +str(accuracy)+"_"+"epoth_"+ str(epoch) + "_model" + ".pth.tar"
	# torch.save(model.state_dict(),exModelName)
	torch.save({'cfg': cfg, 'state_dict': model.state_dict()}, exModelName)
...
if __name__=='__main__':
        ...
        modelPath = r"D:\trainTemp\handBag\res50\0.8260869565217391_epoth_112_model.pth.tar"
        checkPoint = torch.load(modelPath)
        # cfg = checkPoint["cfg"]
        # print(cfg)
        model = myNet(num_classes=opt.numClasses)
        techermodel =resnet50()
        techermodel.cuda()
        model_dict = checkPoint['state_dict']
        techermodel.load_state_dict(model_dict)
        trainset = dset.ImageFolder(r'D:\trainTemp\handBag\train', transform=transform_train, loader=cv_imread)
        print(trainset[0][0])

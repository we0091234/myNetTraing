...
class myNetT(nn.Module):
    def __init__(self,cfg=None,num_classes=3):
        super(myNetT, self).__init__()
        if cfg is None:
            cfg = myCfg
        self.feature = self.make_layers(cfg, True)
        self.fc1=nn.Linear(cfg[-1], 2)
    ...
    def forward(self, x):
        ...
        z = x.view(x.size(0), -1)
        center_feature=self.fc1(z)
        x = x.view(x.size(0), -1)
        y = self.classifier(center_feature)
        if self.training:
            return center_feature,y
        else:
            return y

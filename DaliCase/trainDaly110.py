...
def create_dali_pipeline(data_dir, crop, size, shard_id, num_shards, dali_cpu=False, is_training=True):
    images, labels = fn.readers.caffe(path=data_dir,
                                     shard_id=shard_id,
    ...
    if is_training:
        images = fn.decoders.image_random_crop(images,
                                               device=decoder_device, output_type=types.BGR,
...
def main():
    ...
    if len(args.data) == 1:
        ...
        valdir= args.data[1]
    traindir="/home/xiaolei/ramdisk/Gender_lmdb"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        [prec1, prec5] = validate(valloader, model, criterion)
        savePath = "./modelGender"
        if not os.path.exists(savePath):
            os.mkdir(savePath)
        modelName = str(format(prec1,'.6f'))+"_"+str(epoch)+".pth.tar"

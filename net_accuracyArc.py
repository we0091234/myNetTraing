...
# opt.meanfile=r'F:\@Pedestrain_attribute\@_pedestrain2\NoStdVehicle.npy'
...
opt.testfile=r"H:\daTUtest\zxc_st\@_DriverTest\shenzhenBelt\test"
# opt.testfile=r"C:\train_data\NSPedestrain\shoutuiCar\val"
...
opt.savefile =r"H:\daTUtest\zxc_st\@_DriverTest\shenzhenBelt\test"
opt.modelfile = r"F:\@Pedestrain_attribute\@_driver_combined\@_release\@release0608\0.940517_epoth_52_model.pth.tar"
# opt.isParallel = True
...
if not os.path.isfile(opt.modelfile):#not opt.modelfile.endswith("pth.tar"):
    ...
    for modeli in modellist:
        ...
        if opt.cfg == True:
            ...
            model = myNetFace(num_classes=opt.numclass, cfg=cfg)
            # model =resnet50()
        else:
            model = resnet18()
        if opt.isParallel:
